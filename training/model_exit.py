...
# Smaller architecture than entry model - exit features are more focused.
# 18 inputs -> 64 -> LayerNorm -> ReLU -> Dropout(0.3) -> 32 -> LayerNorm -> ReLU -> Dropout(0.3) -> 1
# Output is predicted remaining upside (continuous, >= 0).
class ModelExit(nn.Module):

    def __init__(self, input_size):
        super(ModelExit, self).__init__()
        self._hidden1 = nn.Linear(input_size, 64)
        self._ln1     = nn.LayerNorm(64)
        self._hidden2 = nn.Linear(64, 32)
        self._ln2     = nn.LayerNorm(32)
...
class TrainerExit:
    ...
    def __init__(self, model, device, learning_rate, swa_start=3, swa_end=None):
        self._device     = device
        self._model      = model.to(device)
        self._swa_start  = swa_start
        self._swa_end    = swa_end
        # HuberLoss: treats small errors as MSE, large errors as MAE.
        # delta=0.2 means errors within 20% return use MSE, beyond use MAE.
        # This prevents extreme remaining_upside labels (200%+ contracts) from
        # dominating gradients while still learning from them.
        self._criterion  = nn.HuberLoss(delta=0.2)
