# Almost the same as evalutaion.py. 
# Only differece is that this tries dynamic exits rather than fixed.
...
class Backtester:
    ...
    #   - Look up (predicted_remaining_upside, unrealized_return) for (entry_timestamp, today)
    #   - unrealized_return from the parquet is the actual return at that point in time -
    #     no look-ahead. Falls back to pos['trade_return'] only if no lookup entry exists.
    #   - Emergency exit if unrealized > EMERGENCY_MIN and SPY just dropped sharply
    #   - Exit if predicted_remaining < unrealized * EXIT_RATIO
    #   - Otherwise hold to window end
    def run_with_exit(
        self,
        ...
        # Load exit model predictions
        # Key: (entry_timestamp_str, date_str) -> (predicted_remaining_upside, unrealized_return)
        # Vectorized build - iterrows on 1M+ rows is too slow.
        exit_lookup = {}
        try:
            exit_df = pd.read_parquet(exit_predictions_path)
            exit_df['entry_str'] = exit_df['entry_timestamp'].apply(lambda x: str(pd.Timestamp(x)))
            exit_df['date_str']  = exit_df['current_timestamp'].apply(lambda x: str(pd.Timestamp(x).normalize()))
            exit_lookup = dict(zip(
                zip(exit_df['entry_str'], exit_df['date_str']),
                zip(exit_df['predicted_remaining_upside'], exit_df['unrealized_return'])
            ))
        ...
        for i in range(len(predictions)):
            ...
            for pos in open_positions:
                ...
                # Window expired - always close at day-21 realized return
                if current_time >= pos['close_time']:
                    capital = self._close_position(pos, pos['trade_return'], capital, trade_log)
                ...
                # Look up actual unrealized return and exit prediction for today.
                # unrealized_return in the parquet is the real return at current_timestamp -
                # no look-ahead bias. Falls back to day-21 return only if no lookup exists
                # (e.g. position is a loser with no exit sequence generated).
                lookup_result = exit_lookup.get((entry_str, current_date))
                if lookup_result is not None:
                    pred_remaining, unrealized = lookup_result
                else:
                    pred_remaining = None
                    unrealized = pos['trade_return']

                # Emergency exit: SPY dropped sharply and position has meaningful gains
                ...
                # Exit model trigger: predicted remaining upside is small relative to gains
