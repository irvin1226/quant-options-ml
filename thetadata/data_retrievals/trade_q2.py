...
def save_trade(date: str):
    ...
    trade_dir = os.path.join(day_dir, "trade")
    os.makedirs(trade_dir, exist_ok=True)

    for expiration in expirations:
        out_path = os.path.join(trade_dir, f"trade_{expiration}.csv")
        if os.path.exists(out_path):
            continue
        
        # API Call
        response_text = client.get_trade(date, expiration)
        if response_text == "":
            continue
...
def main():
    ...
    q2 = ['04', '05', '06']
    q2_dates = df[months.isin(q2)]

    for date in q2_dates.iloc[:, 0]:
        save_trade(date)
